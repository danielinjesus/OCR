...
def get_cosine_map(embed, threshold = 0.85, imgfname = None):
    step = torch.cat((embed[:,:,1:,:], embed[:,:,-1:,:]), dim=2) # y+1
    step = torch.cat((step[:,:,:,1:], step[:,:,:,-1:]), dim=3) # x+1
    assert step.shape == embed.shape
    cosine_map = torch.nn.functional.cosine_similarity(embed, step, dim=1, eps=1e-8) # [-1,1]

    if imgfname is not None:
        cosine_map = (cosine_map - 1) * -0.5 # [1,0]
        # cosine_map = cosine_map * (conf.detach() > 0.5)
        cosine_map = torch.nn.functional.max_pool2d(cosine_map, 2, stride = 1)
        cosine_map = cosine_map / torch.max(cosine_map) 
        cosine_map = torch.clamp(cosine_map, 0, 1)
        vis_map = cosine_map[0].detach().cpu().numpy() * 255
        vis_map = vis_map.astype(np.int32)
        # print(vis_map.shape)
        cv2.imwrite("vis/cosine_map/"+imgfname,vis_map)
        return cosine_map
    return torch.clamp((cosine_map+1)/ (2* torch.max(cosine_map)), 0, 1)