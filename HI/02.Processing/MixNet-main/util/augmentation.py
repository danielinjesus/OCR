...
class Augmentation(object):
    def __init__(self, size, mean, std):
        ...
        self.augmentation = Compose([
            ...
            RandomResizedCrop(),
            RotatePadding(up=60, colors=False),  # pretrain on Syn is "up=30", else is "up=60"
...
class ImageFormatCorrection(object):
    """이미지 형식을 표준 PyTorch 형식(NCHW)으로 변환합니다."""
    
    def __call__(self, image, polygons=None):
        # 이미지가 텐서인 경우
        if isinstance(image, torch.Tensor):
            # NHWC 형식인 경우(채널이 마지막 차원) NCHW로 변환
            if image.dim() >= 3 and image.shape[-1] == 3:
                image = image.permute(0, 3, 1, 2) if image.dim() == 4 else image.permute(2, 0, 1)
                
        # 이미지가 NumPy 배열인 경우
        elif isinstance(image, np.ndarray):
            # HWC 형식인지 확인 (채널이 마지막 차원)
            if image.ndim == 3 and image.shape[2] == 3:
                # NumPy 배열을 PyTorch 텐서로 변환 (CHW 형식으로)
...
class BaseTransform(object):
    def __init__(self, size, mean, std):
        ...
        self.augmentation = Compose([
            ...
            Normalize(mean, std),
            ImageFormatCorrection(),
        ])

    def __call__(self, image, polygons=None):
        # 이미지가 이미 PyTorch 텐서인 경우 NumPy 배열로 변환
        if isinstance(image, torch.Tensor):
            # 텐서를 NumPy 배열로 변환 (OpenCV 함수에 사용하기 위해)
            if image.dim() == 3 and image.shape[0] == 3:  # CHW 형식
                image = image.permute(1, 2, 0).numpy()  # HWC로 변환
            elif image.dim() == 3 and image.shape[2] == 3:  # HWC 형식
                image = image.numpy()
