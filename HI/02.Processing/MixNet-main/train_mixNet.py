...
from dataset.custom_dataset import CustomTextDataset, collate_fn
...
def train(model, train_loader, criterion, scheduler, optimizer, epoch):
    ...
    for i, inputs in enumerate(train_loader):
        if inputs is None:  # collate_fn에서 None을 반환할 경우 건너뜀
            continue
            
        data_time.update(time.time() - end)
        train_step += 1
        
        # CustomDataset의 경우 inputs가 dict 형태일 수 있음
        if isinstance(inputs, dict):
            input_dict = inputs
            # dict 형태의 inputs를 device로 이동
            for k, v in input_dict.items():
                if isinstance(v, torch.Tensor):
                    input_dict[k] = v.to(cfg.device)
        else:
    ...
    elif cfg.exp_name == 'MLT2019' or cfg.exp_name == 'ArT' or cfg.exp_name == 'MLT2017' or cfg.exp_name == 'CustomDataset':  # CustomDataset 추가
...
def main():
    ...
    if cfg.exp_name == 'Totaltext':
        ...
        valset = None
    elif cfg.exp_name == 'CustomDataset':
        trainset = CustomTextDataset(
            data_root='/data/ephemeral/home/industry-partnership-project-brainventures/data/Fastcampus_project/images/train',
            json_path='/data/ephemeral/home/industry-partnership-project-brainventures/data/Fastcampus_project/jsons/train_mixnet_format.json',
            is_training=True,
            load_memory=cfg.load_memory,
            transform=Augmentation(size=cfg.input_size, mean=cfg.means, std=cfg.stds)
        )
    ...
    # CustomDataset일 경우 특별한 collate_fn 사용
    if cfg.exp_name == 'CustomDataset':
    ...
    if cfg.know:
        ...
        know_model = know_model.to(cfg.device)
        know_model.eval()
    ...
    lr = cfg.lr
    moment = cfg.momentum
    if cfg.optim == "Adam" or cfg.exp_name == 'Synthtext' or cfg.exp_name == 'CustomDataset':  # CustomDataset 추가
