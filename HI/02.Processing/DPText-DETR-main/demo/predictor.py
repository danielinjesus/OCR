...
class VisualizationDemo(object):
    ...
    def extract_polygon_list(self, instances):
        """
        인스턴스에서 다각형 좌표를 추출하여 요청된 형식의 문자열로 반환합니다.
        형식: "x1 y1 x2 y2 ... xn yn|x1 y1 x2 y2 ... xn yn"
        
        Args:
            instances: 모델이 예측한 인스턴스들
            
        Returns:
            str: 다각형 좌표들을 파이프(|)로 구분한 문자열
        """
        polygon_list = []
        
        if not instances.has("polygons"):
            return ""
        ...
        for polygon in polygons:
            # 각 다각형의 좌표를 정수로 변환하고 문자열로 포맷합니다
            coords = polygon.cpu().numpy().astype(int)
            # 모든 좌표를 공백으로 구분하여 연결합니다
            polygon_str = " ".join(map(str, coords))
            polygon_list.append(polygon_str)
        
        # 모든 다각형을 파이프(|)로 구분하여 연결합니다
        return "|".join(polygon_list)
    ...
    def run_on_image(self, image):
        ...
        if "panoptic_seg" in predictions:
            ...
            if "instances" in predictions:
                ...
                polygon_list = self.extract_polygon_list(instances)

        return predictions, vis_output, polygon_list
