...
import sys
sys.path.append("/data/ephemeral/home/DPText-DETR-main")
from fastcampus_dataset import register_fastcampus_datasets
...
from adet.data.dataset_mapper import DatasetMapperWithBasis, SafeDatasetMapperWithBasis
...
class Trainer(DefaultTrainer):
    ...
    def train_loop(self, start_iter: int, max_iter: int):
        """
        Override the train_loop to add more logging
        """
        print(f"Starting training from iteration {start_iter} to {max_iter}")
        
        # DefaultTrainer.train_loop을 호출하는 대신 직접 구현
        ...
        with EventStorage(start_iter) as self.storage:
            try:
                ...
                for self.iter in range(start_iter, max_iter):
                    if self.iter % 10 == 0:  # 10 iteration마다 출력
                        print(f"Training iteration: {self.iter}/{max_iter}")
                    self.before_step()
                    self.run_step()
                    self.after_step()
                    if self.iter % 100 == 0:  # 100 iteration마다 더 상세한 출력
                        print(f"Loss stats: {self.storage.latest()}")
                # self.iter == max_iter + 1 해야 마지막 checkpoint가 저장됩니다
                self.iter += 1
            finally:
                self.after_train()
    ...
    def build_train_loader(cls, cfg):
        ...
        """
        try:
            print("Using SafeDatasetMapperWithBasis")
            mapper = SafeDatasetMapperWithBasis(cfg, True)
        except ImportError:
            print("Using DatasetMapperWithBasis ")
    ...
    def build_evaluator(cls, cfg, dataset_name, output_folder=None):
        ...
        print(f"Building evaluator for dataset: {dataset_name}")
