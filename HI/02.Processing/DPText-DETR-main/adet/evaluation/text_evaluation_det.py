...
class TextDetEvaluator(DatasetEvaluator):
    ...
    def __init__(self, dataset_name, cfg, distributed, output_dir=None):
        ...
        elif "fastcampus" in dataset_name:  # 새로운 조건 추가
            # 1. FastCampus 데이터셋용 gt zip 파일이 있으면 경로 지정
            # self._text_eval_gt_path = "datasets/evaluation/gt_fastcampus.zip"
            
            # 2. 또는 평가를 건너뛰려면:
            self._text_eval_gt_path = None
            self.submit = True  # 결과만 저장하고 평가는 건너뜀
        else:
