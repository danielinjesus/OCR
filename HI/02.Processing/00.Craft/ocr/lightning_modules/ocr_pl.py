...
class OCRPLModule(pl.LightningModule):
    ...
    def validation_step_pic(self, batch, batch_idx, pred):
            # 첫 번째 이미지만 처리
            img = batch['images'][0].cpu().permute(1, 2, 0).numpy()
            img = (img * 0.229) + 0.485  # 정규화 해제 (대략적)
            
            # 점수 맵
            region = pred['region_score'][0, 0].cpu().detach().numpy()
            affinity = pred['affinity_score'][0, 0].cpu().detach().numpy()
            ...
            plt.subplot(2, 2, 2)
            plt.title(f'Region Score (min={region.min():.4f}, max={region.max():.4f})')
            plt.imshow(region, cmap='jet')
            
            plt.subplot(2, 2, 3)
            plt.title(f'Affinity Score (min={affinity.min():.4f}, max={affinity.max():.4f})')
            plt.imshow(affinity, cmap='jet')
            
            # 히스토그램 추가
            plt.subplot(2, 2, 4)
            plt.title('Score Histograms')
            plt.hist(region.flatten(), bins=50, alpha=0.5, label='Region')
            plt.hist(affinity.flatten(), bins=50, alpha=0.5, label='Affinity')
            plt.legend()
            
            plt.savefig(f'debug_viz/maps_batch_{batch_idx}.png')
            plt.close()
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.validation_step_pic(batch, batch_idx, pred)
