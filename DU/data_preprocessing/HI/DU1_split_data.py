    ...
    for filename in os.listdir(directory_path):
        if filename.endswith('.json'):
            ...
            try:
                ...
                subclass = "Horizontal"  # 기본값 설정
                parts = filename.split('_')
                if len(parts) >= 2:
                    subclass = parts[1]

                if subclass:
                    json_data[subclass][filename] = remove_duplicates_in_annotations(data)
                    file_count += 1
    ...
    for ext in extensions:
        ...
        if os.path.exists(src_path):
            try:
                ...
                # 이미지 파일이 유효한지 확인 (PIL 사용)
                from PIL import Image
                img = Image.open(src_path)
                img.verify()  # 이미지 파일 검증
                # 메모리에서 닫고 다시 열어 실제로 사용할 수 있는지 확인
                img.close()
                img = Image.open(src_path)
                img.load()
                img.close()
                ...
                return True, src_path, dst_path
            except FileNotFoundError as e:
                tqdm.write(f"Error: 파일을 찾을 수 없음 - {e}")
                return False, None, None
            except (IOError, SyntaxError) as e:
                tqdm.write(f"Error: 이미지 파일 손상됨 - {base_name + ext}: {e}")
                return False, None, None
            except Exception as e:
                tqdm.write(f"Error: 알 수 없는 오류 발생 - {base_name + ext}: {e}")
                return False, None, None
...
def split_train_test_val(json_data, export_count_list, src_image_dir, json_dir, image_dir):
    ...
    total_files = sum(count for count in export_count_list.values())

    with tqdm(total=total_files, desc="데이터셋 분할 및 이미지 이동") as pbar:
        for subclass, count in export_count_list.items():
            ...
            if subclass in json_data and count > 0:
                ...
                for file_name in tqdm(train_files, desc=f"{subclass} - Train", leave=False):
                    if file_name in json_data[subclass]: 
                        ...
                        if not found:
                            tqdm.write(f"경고: 이미지 파일을 찾을 수 없음: {base_name}.*")
                        else:
                            train_data[file_name] = json_data[subclass][file_name]
                            stats['train'] += 1
                            subclass_stats[subclass]['train'] += 1 
                
                for file_name in tqdm(test_files, desc=f"{subclass} - Test", leave=False):
                    if file_name in json_data[subclass]:    
                        # 이미지 파일 이동
                        base_name = file_name.replace('.json', '')
                ...
                for file_name in tqdm(val_files, desc=f"{subclass} - Val", leave=False):
                    if file_name in json_data[subclass]:
                        # 이미지 파일 이동
                        base_name = file_name.replace('.json', '')
    ...
    with open(os.path.join(json_dir, 'train.json'), 'w', encoding='utf-8') as f:
        json.dump(convert_data_format(train_data), f, ensure_ascii=False, indent=2)
   
    with open(os.path.join(json_dir, 'val.json'), 'w', encoding='utf-8') as f:
        json.dump(convert_data_format(val_data), f, ensure_ascii=False, indent=2)

    with open(os.path.join(json_dir, 'test.json'), 'w', encoding='utf-8') as f:
        json.dump(convert_data_format(test_data), f, ensure_ascii=False, indent=2)
...
if __name__ == "__main__":
    load_json_path = '/data/ephemeral/home/Fastcampus_project/Fastcampus_project/json'
    load_image_path = '/data/ephemeral/home/Fastcampus_project/Fastcampus_project/image'


    output_json_dir = '/data/ephemeral/home/Fastcampus_project/Fastcampus_project/jsons'
    output_image_dir = '/data/ephemeral/home/Fastcampus_project/Fastcampus_project/images'
