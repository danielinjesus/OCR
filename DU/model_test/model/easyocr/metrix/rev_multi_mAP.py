...
def calculate_batch_metrics(gt_paths, pred_paths):
    """
    1000장 이미지의 평균 AP와 영역 기반 IoU를 계산
    """
    all_aps = []
    all_area_ious = []
    ...
    for gt_path, pred_path in zip(gt_paths, pred_paths):
        ...
        # 각 이미지별 AP와 영역 기반 IoU 계산
        ap, _, _ = calculate_map(gt_boxes, pred_boxes, confidence_scores)
        area_iou = calculate_area_based_map(gt_boxes, pred_boxes, confidence_scores)
        
        all_aps.append(ap)
        all_area_ious.append(area_iou)
        
    # 전체 평균 계산
    mean_ap = np.mean(all_aps)
    mean_area_iou = np.mean(all_area_ious)
    
    print("\n전체 이미지 평가 결과:")
    print(f"Mean AP: {mean_ap:.4f}")
    print(f"Mean Area-based IoU: {mean_area_iou:.4f}")
    
    return mean_ap, mean_area_iou

# 사용 예시:
gt_paths = [f"/data/ephemeral/home/industry-partnership-project-brainventures/DU/model_test/data_sample/label/{i:06d}.json" for i in range(1, 1001)]
pred_paths = [f"/data/ephemeral/home/industry-partnership-project-brainventures/DU/model_test/output/easyocr/annotated_책표지_총류_{i:06d}.json" for i in range(1, 1001)]

mean_ap, mean_area_iou = calculate_batch_metrics(gt_paths, pred_paths)